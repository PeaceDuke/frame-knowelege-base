...
namespace ItemPlacementKnowlegeBase
{
    public partial class Form_edit_slot : Form
    {
        ...
        {
            cb_type.Items.Clear();
            cb_type.Items.AddRange(new string[] { "Int32", "Single", "String", "Frame" });

            cb_value.Items.Clear();
            var frames = Knowleges_edit.Frames.FindAll(x => x.Name != cur_frame.Name);
        ...
        private void Bt_applay_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (cb_type.SelectedIndex)
                {
                    case 0:
                        type = typeof(int);
                        int value_i = Int32.Parse(tb_value.Text);
                        if(form_type == FormType.Insert)
                            Knowleges_edit.GetFrame(cur_frame.Name).AddSlot(name_slot, value_i, type);
                        else
                        {
                            Slot sl = new Slot(cur_slot.Name, value_i, type);
                            Knowleges_edit.GetFrame(cur_frame.Name).SetSlot(sl);
                        }
                        break;
                    case 1:
                        type = typeof(float);
                        float value_f = float.Parse(tb_value.Text);
                        if (form_type == FormType.Insert)
                            Knowleges_edit.GetFrame(cur_frame.Name).AddSlot(name_slot, value_f, type);
                        else
                        {
                    ...
                    case 2:
                        type = typeof(string);
                        string value_s = tb_value.Text;
                        if (form_type == FormType.Insert)
                            Knowleges_edit.GetFrame(cur_frame.Name).AddSlot(name_slot, value_s, type);
                        else
                        {
                    ...
                    case 3:
                        ...
                        if (form_type == FormType.Insert)
                            Knowleges_edit.GetFrame(cur_frame.Name).AddSlot(name_slot, value_fr, type);
                        else
                        {
        ...
        private void Form_edit_slot_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible == true)
            {
                ...
                if (form_type == FormType.Update)
                {
                    ...
                    cb_type.SelectedIndex = cb_type.FindString(cur_slot.ValueType.Name);
                    if (cb_type.SelectedIndex == 3)
                    {
                        cb_value.SelectedIndex = cb_value.FindString(cur_slot.Value.ToString());
                    }
                    else
                    {
                        tb_value.Text = cur_slot.Value.ToString();
                    }
